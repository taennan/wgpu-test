use crate::{
    graphics::texture_v2::{texture_buffer_pool::TextureBufferPool, utils},
    utils::paths,
};
...
use wgpu::{
    AddressMode, CommandEncoder, Device, FilterMode, Sampler, SamplerDescriptor, TextureDescriptor,
...
type AtlasAllocation = (UVec2, HashMap<PathBuf, AtlasItem>);
...
impl From<&Allocation> for AtlasItem {
...
impl TextureAtlas {
    ...
    pub fn get_texture_uv_offset(&self, texture_path: &PathBuf) -> Option<Vec2> {
        self.items.get(texture_path).map(|item| {
            ...
            Vec2::ONE / (self_size / item_pos)
        })
    }

    pub fn get_texture_uv_size(&self, texture_path: &PathBuf) -> Option<Vec2> {
        self.items.get(texture_path).map(|item| {
            let self_size = Vec2::new(self.size().x as f32, self.size().y as f32);
            let item_size = Vec2::new(item.size.x as f32, item.size.y as f32);
            Vec2::ONE / (self_size / item_size)
        })
    }
    ...
    fn re_init(
        ...
        let (size, items) = Self::allocate_items(&reinit_paths, buffers, device);
        self.items = items;
        self.size = size;
        ...
        P: AsRef<Path> + Into<PathBuf> + Clone,
    {
        ...
        for (index, image) in images.iter().enumerate() {
            ...
            let atlas_item = AtlasItem::from(&allocation.expect("Image was not allocated"));
    ...
    where
        P: AsRef<Path>,
    {
        let image_path = paths::assets().join(image_path);
        let encoded_image = match ImageReader::open(&image_path) {
            Ok(image) => image,
            Err(_) => {
                log::error!("Failed to open image at '{:?}'", image_path);
                ImageReader::open(&paths::assets().join("albatross-light.jpg"))
